...
| I20 | CI advertises no artifact: `flatpak.yml` triggers on tags but uploads nothing, and the tag build is the only thing that would produce a shippable bundle. Either add an upload step or stop triggering on tags (T14, packaging.md §2.5) | — | ☑ fixed in T21, arm (a): tag-gated `build-bundle` from the retained OSTree repo + attach to the Release (`contents: write` job-scoped); tags trigger kept, arm (b) rejected; `workflow_dispatch`/fork-tag observation left as release-checklist items |
...
| I29 | **Five parity rows regressed to `bug` in the walk** — each a frozen row whose *live* half works but whose named affordance is absent, with no ux.md decision authorising the omission. **#13** "Check Again" is inert for the process lifetime: the button sends `ContainerMsg::RefreshRequested`, which re-runs `backend.containers()` but never re-probes the environment, because `EnvGuard` is built once in `Application::init` (`app.rs:425`) and `Backend` exposes no `set_env`/re-probe API — so a user who installs distrobox while the app is open is told to check again, and checking again changes nothing. **#20** the task card renders no spinner and no "In progress…/Completed" copy while its own doc comment (`views.rs:522`) claims "spinner while running"; running is signalled only by the Cancel button's presence. **#97** the Images page's Refresh is the *global containers* refresh (`app.rs:1544-1548` → `refresh_containers` only), so `backend.images()` — one call site, `app.rs:234` — can never be re-fetched after its single lazy load; the tree already special-cases Apps for exactly this mis-wiring (`app.rs:1537-1540`) and did not for Images. **#134** the FAB bug is only half fixed: the FAB is gone, but in the blocked and not-installed states the shell gate replaces the page while `header_end` still renders "New Snapshot" (`app.rs:1582-1588`), which can only toast "Select a container first." — the button is visible in precisely the states the row called out, and the `app.rs:1575` comment claiming the bug "disappears with the move" overstates. **#186** `status_color` (`icons.rs:96`) is `#[allow(dead_code)]` and **callerless** — a repo-wide grep finds only its definition, its four test assertions, and a comment saying "NOT YET RENDERED" — so the theme-role mapping is tested but no colour reaches any widget, and `distro_colour` was never written at all (T16 walk). **#20's testability improved, the bug did not.** After I31's lib target, `views::task_affordance` factors the row's trailing-affordance decision into a pure value and `app/tests/parity_rows.rs` pins all four flag combinations — so #20 now has executable evidence (`TaskAffordance::RunningCancelOnly` literally names the cancel-only running state) instead of only a doc comment asserting a spinner. **No spinner was added:** choosing between adding it and dropping the row's claim is a product decision, and the variant name is deliberately chosen so that making either choice forces a test edit. **#13 and #97 closed by T17:** "Check Again" re-probes via `Backend::reprobe()` + a live `Probed` arm (T2 flip test, T1 outcome table), and the Images header Refresh routes to `backend.images()` through `views::header_refresh` (T1 ten-page table); appendix rows re-tiered with evidence, aggregate counts reconcile in T21 | #13 #20 #97 #134 #186 | ◐ #13/#97 closed, #20/#134/#186 open |
| I30 | **Seven parity rows are `missing`** — absent, not rescoped, and each needs a wire-or-drop ruling rather than a silent pass. **#5** per-destination nav icons: the single `nav_model.insert()` call (`app.rs:421`) never chains `.icon()`, so the rail is text-only for all ten destinations and `Page::title()` has no `Page::icon()` sibling; the `Entity::icon` API exists at the pinned rev. **#40** the running-only inline "Open Terminal" on the container *card* — the details page has the same control with the same running gate, but the card offers no terminal path. **#41** the card's ⋮ quick-actions trigger and its sheet: no `context_menu`, `popover` or `context_drawer` exists anywhere in `app/src`, and `app.rs:3545-3548` explicitly defers a card-menu shell. **#161** task history is still in-memory only — ux.md:266 classes persistence as P1, and the Activity page shows the live session exactly as Flutter did. **#185** the colour duplication §3.6 asked to consolidate: `status_color` is callerless (see I29/#186) and `distro_colour` was never written, so the ×3 distro-colour and ×4 status-colour duplication is unresolved while the icon/label halves did land. **#189** keyboard shortcuts: `keyboard_nav::subscription()` and `set_keyboard_nav` are verified present at the pinned rev; nothing in `app/` uses them, so the Ctrl+R the row's approach column names does not exist. **#190** a11y semantics: §5.3's priority-1 item (labels on icon-only controls) is untouched and the Orca acceptance gate was never run (T16 walk). **#185 closed by T18** (`distro_colour` written, both helpers reach widgets). **#5/#40/#41/#189/#190 closed by T19:** `Page::icon()` + nav `.icon()` chain (names verified in pop-icon-theme), running-only card terminal + ⋮ drawer reusing existing arms, `set_keyboard_nav` + Escape/Search hooks + Ctrl+R/Ctrl+N listener, `IconControl` labels on both icon-only controls (T1 pins; Orca gate filed as a finding — unrunnable headless, R13 stays open); appendix rows re-tiered with evidence, aggregate counts reconcile in T21 | #5 #40 #41 #161 #185 #189 #190 | ☑ closed — **#161 closed by T20** (50-entry `task_history` ring in cosmic-config + init seeding so Activity renders persisted + live; T2 restart round-trip, T1 bound/eviction/migration pins); appendix row re-tiered with evidence, aggregate counts reconcile in T21 |
| I31 | **The app crate's missing lib target is a systemic verification ceiling, not a page-level gap.** `app/` declares only `[[bin]]` and has no `src/lib.rs`, so no integration test can import `App`, any page module, or the widget tree. Consequence measured across the walk: of 193 rows, **148 rest on `source` tier** (a verified call path read in the code, not an executed behaviour), against 26 T1 and 12 T2. Every page-level row is affected, and the two tiers that do exist are reachable only because the row's subject happened to coincide with a pure helper or a `core`-side backend contract. The walk's adversarial pass found that **14 of 193 rows carried inflated tiers** for exactly this reason — a T2 recorded for a row whose cited test exercised a *neighbouring* core type or a backend argv, never the row's own subject. Fixing this properly means either promoting `app/` to a lib + thin bin (so integration tests can construct `App`) or adding a headless render harness; both are out of T16's scope and are recorded here rather than silently absorbed into tier counts (T16 walk). **The structural half is fixed.** `app/` is now a lib + thin bin: `app/src/lib.rs` declares the same module tree `main.rs` used to (main.rs keeps only the entry point), and `app/Cargo.toml` gained a `[lib]` target. An integration test can therefore import the crate for the first time — `app/tests/parity_rows.rs` is the first one that does, pinning the nav rail's ten destinations and order (I26), the distinct-title property, `TaskMsg::Completed`'s `success` flag (#21), and the task row's affordance decision across all four flag combinations (#20). Two of those assertions kill a real mutation: mapping `(true,false)` to `Succeeded` (row #21 regressing to a success check on a failed task) and letting `success` influence the running branch. **What is deliberately NOT claimed:** this lifts the *ceiling*, it does not raise the 148 rows' tier. Those rows each still need a test written against them, and no tier count in REPORT.md was changed by this commit. The other half — a headless render harness that constructs `App` (which needs a `cosmic::app::Core` and a running executor) — remains open and is what would make the *rendered* output assertable rather than the decisions behind it. Nor is any user-facing bug fixed: #20's missing spinner is still missing, and is now named in the type system (`TaskAffordance::RunningCancelOnly`) instead of only in a doc comment that claims the opposite | — | ☑ closed — **harness half landed in T20** (`app/tests/render_harness.rs`: `Core::default()` + `init` + driven `update` + headless `view` over a `HarnessSnapshot` seam, `into_stream` draining for done-shaped tasks under `#[tokio::test]`; seed rows #161/#21/#152/#153/#154 + executor proof + T2 restart round-trip; `Element` stays opaque so pixels still need T3 and no `source` row moves without its own test) |
| I26 | **The app ships 10 nav destinations; `ux.md` §4.2 authorises 8.** `ux.md:68` says "**8 destinations:** keep all 8, same order and icons (Dashboard, Containers, Images, PackageManager, Updates, Backups, ActivityLogs, Settings)" — which is exactly `home_screen.dart:76-83`, the entire Flutter rail. `app/src/views.rs:53` (`Page::ALL`) has those 8 **plus `Apps` and `Stats`**. Neither was in the rail: `apps_page.dart` was a pushed route from the details page, and nothing in the Flutter tree renders a "Stats" destination at all (the only `Stats` token is `app_state.dart`'s `containerStats`/`loadContainerStats`, the data method the details sheet used). So both are additions the port made, not parity. `Stats` is not ux.md's warned-off placeholder: `app.rs:344 view_stats()` renders real measured values (`stats.cpu_percent` formatted at the call site into a placeable, per §Q17). `ux.md:262` offered "a nav destination or (better) a section on Settings" and the port took the first. Consequence to settle in the walk: whether a destination reachable in neither Flutter nor the parity rows belongs in the chrome, and whether `Apps` (contextual, a pushed route in Flutter) should stay top-level when `Details` and `Terminal` — the other two pushed routes, per `ux.md:74` — did **not** get destinations. Flagged rather than silently kept (T15). **Now pinned, still undecided:** `app/tests/parity_rows.rs` (possible only after I31's lib target) asserts the ten destinations *and their order* — the order is load-bearing because `activate_page` resolves a page with `Page::ALL.iter().position(...)` and feeds the index to `nav_model.activate_position(pos as u16)`, so a silent reorder activates the wrong tab. A second test pins that the eight `ux.md` destinations are all still present and that the only additions remain `Apps` and `Stats`, so I26 cannot be closed by dropping a Flutter destination instead of ruling on the two additions. The decision itself — does a destination neither Flutter nor the parity rows authorise belong in the chrome, and should contextual `Apps` stay top-level when `Details`/`Terminal` did not — is still open. **Ruled by D28, applied by T19:** Apps demoted to a details-pushed route (as in Flutter), Stats stays as the single deliberate addition; rail 10→9, `parity_rows.rs` pins the ruled 9 + order + the 8+Stats property + an Apps-absent-from-rail guard | — | ☑ ruled (D28), pinned |
| I32 | **Ctrl+N and the Dashboard quick action silently wipe an open wizard.** T19's devil's-advocate review filed this as blocking objection B1 (Ctrl+N unconditionally delegates to `NewContainerRequested`, whose arm unconditionally runs `self.wizard = Some(WizardState::default())`); no fix commit landed between T19 and the final pass, and the final pass re-verified it at HEAD (`app.rs:1589-1591` → `app.rs:955-972`) — and widened it. Only the *header* button is guarded (`wizard.is_none()`, `app.rs:1840`); the Dashboard quick action (`views.rs:779-780`, unconditional `.on_press`) is a second unguarded path, reachable because the wizard renders only on `Page::Containers` (`app.rs:1699`) while nav stays live — open the wizard, switch to Dashboard, press New Container, lose the form (including an Images #95 preselect). The Images preselect arms (`app.rs:1333,1347`) also overwrite unconditionally; arguably intended (they carry an image), but they wipe a half-filled form the same way. Fix direction: no-op when `self.wizard.is_some()` on the two blank-wizard paths (mirror the header condition); confirm-or-keep for the preselect arms. One-line-class fix, filed not fixed (final pass is docs-only) | — | ☐ |
| I33 | **The create wizard shows an empty image catalogue unless the Images page was visited first.** Images lazy-load on first Images-page visit only (`app.rs:607-613`); the wizard entry (`NewContainerRequested`, `app.rs:955-972`) never triggers `ImageMsg::LoadRequested`. Observed live in the final pass: Ctrl+N opened the wizard to "No images available" with no spinner and no error, while `flatpak-spawn --host distrobox create --compatibility` on the same host returns a non-empty list and the app log shows no `create --compatibility` run at all. The custom-URL escape hatch renders, so creation is not blocked — but the wizard's first step is empty on the primary path. Fix direction: fire `LoadRequested` from the wizard entry when `images` is empty and not loading (same guard as first-visit); the wizard already threads `loading.images` (row-progress) so the in-flight state renders. Filed not fixed (final pass is docs-only) | — | ☐ |
...
| T19 | Nav icons (#5) + card terminal (#40) + card menu (#41) + shortcuts (#189) + a11y labels (#190), on the D28 destination set | ux | **Done** (166+30+5+6+4+6+4+18+7+5+4+5+1 green; 7 new tests + 2 updated I26 pins, each killed under a behaviour-deleting mutation — the Apps-revert mutation kills at compile time via the `[Page; 9]` literal; `verify.sh --fast` stages 1–8 green; I30 #5/#40/#41/#189/#190 → `live`, I26 ruled by D28 with rail 10→9, appendix rows re-tiered, aggregate counts reconcile in T21) |
| T20 | Task-history persistence (#161) + I31 render-harness half | arch | **Done** (171+34+5+6+4+6+4+18+7+7+5+4+5+1 green; 16 new tests + 2 updated settings pins: 5 ring/migration/serde/tail T1s, 2 seed-mapping T1s, 2 cancel-leg pins, 7 headless-harness tests incl. the T2 restart round-trip; 14 killed under a behaviour-deleting mutation each, the init smoke + unknown-cancel negative honestly labelled unkillable-by-deletion; `verify.sh --fast` stages 1–8 green; I30 #161 → `live`, I31 harness half landed with seed rows #161/#21/#152/#153/#154 + executor proof, appendix row re-tiered, aggregate counts reconcile in T21) |
| T21 | CI release artifact (I20) + full gate + REPORT.md appendix | pkg | **Done** (`verify.sh` FULL 11/11 green incl. stages 9–11; stage-8 packaging 25/25 with 8 new CI-YAML assertions, 2 mutation-killed; tag-gated `build-bundle` + Release attach wired, tags trigger kept; local bundle export proof 11 MB from the retained repo; appendix reconciled live 144/rescoped 40/dropped 9, tiers T1 34/T2 13+T1+T2 2/source 143/none 1; I20 closed arm (a), `workflow_dispatch`/fork-tag left as release-checklist items) |
