...
|---|---|---|---|---|
| 1 | 8 nav destinations, fixed order | rescoped | source | Scope grew from 8 to 10: ux.md §1 (line ~76) says "8 destinations: keep all 8, same order and icons". Apps and Stats became nav destinations in the… T19 update: D28 ruled the set to 9 — Apps demoted to a details-pushed route (as in Flutter), Stats kept as the single deliberate addition; `parity_rows.rs` pins the 9 + order + an Apps-absent-from-rail guard. |
...
| 24 | Container row: distro icon, name, status dot, status text, chevron | live | source | Live and shared with the Containers page (one `container_row`, no second copy). T18 update: the ● + status line now rides its theme role via `Text::class`, retiring the "default text colour" caveat. |
...
| 34 | Status colour/label mapping | live | T1 | T18 update: the rescope reason is gone — the colour half is live (`status_color` reaches widgets) alongside the already-live copy-exact label half, and T1 pins both. |
...
| 47 | "Details" row | rescoped | source | Rescoped, not missing: the destination is live and one gesture shallower (tap the card instead of open-a-sheet-then-choose-Details). The Flutter ro… T19 update: the row also ships in the card ⋮ `context_drawer` (#41), T1-pinned per running state. |
| 48 | "Open Terminal" row, disabled when stopped | rescoped | source | Rescoped: the semantics (offered, disabled when stopped) survive verbatim, but on the details page quick-action list (which is §6.5 row #63's surfa… T19 update: the row also ships in the card ⋮ `context_drawer` (#41), T1-pinned per running state. |
| 49 | "Stop Container" row (running only) | rescoped | source | Rescoped to an inline card button; the running-only condition is preserved (views.rs:292), so the Flutter behaviour is intact. One deviation from t… T19 update: the row also ships in the card ⋮ `context_drawer` (#41), T1-pinned per running state. |
| 50 | "Upgrade Container" row | rescoped | source | Rescoped to the details page (which is also §6.5 row #60). The button is the row's subject, so the row itself is source-tier; the underlying task-s… T19 update: the row also ships in the card ⋮ `context_drawer` (#41), T1-pinned per running state. |
| 51 | "Delete Container" row → confirm | rescoped | source | Rescoped: the destructive-class row + confirm pattern is intact and now goes through the one shared ConfirmSpec helper (§3.3) rather than a hand-ro… T19 update: the row also ships in the card ⋮ `context_drawer` (#41), T1-pinned per running state. |
...
| 134 | FAB "New Snapshot" (rendered in all states) [bug row] | live | T1+T2 | Fixed by T18 (was `bug` in the T16 walk): every `header_end` action routes through `views::gate_header_message`, so New Container / Upgrade All / New Snapshot / Apps reload render disabled in the blocked and not-installed states instead of toasting. Refresh stays live (a gated refresh re-probes — T17's recovery path) as does Clear Completed (local mirror). T1: the six-message enabled matrix in `parity_rows.rs`; T2: `header_gate_follows_the_live_backend_env` drives the gate from real blocked/healthy `Backend::env()` guards. |
...
| 190 | a11y semantics / AT support | live | T1 | Fixed by T19 (was `missing` in the T16 walk), with one open gate: §5.3 priority-1 labels are live — both icon-only controls (terminal copy button, card ⋮ trigger) carry the one `IconControl` label into the a11y tree via `description()` plus a sighted tooltip. T1: `every_icon_only_control_has_a_distinct_non_empty_label`. The §5.3 Orca acceptance gate is recorded as a finding, not a pass: no Orca on this host and no display to run it under (§4.4 class) — R13 stays open as the release-gate owner. |
