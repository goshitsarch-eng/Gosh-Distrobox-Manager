...
pub struct CommandRunner {
    pub inner: Arc<dyn InnerCommandRunner>,
    pub output_tracker: OutputTracker<CommandRunnerEvent>,
}

impl CommandRunner {
    pub fn new(inner: Arc<dyn InnerCommandRunner>) -> Self {
        CommandRunner {
            inner,
    ...
    pub fn new_null() -> Self {
        CommandRunner::new(Arc::new(NullCommandRunner::default()))
    }
    pub fn new_real() -> Self {
        CommandRunner::new(Arc::new(RealCommandRunner {}))
    }

    pub fn map_cmd(&self, f: impl Fn(Command) -> Command + Send + Sync + 'static) -> CommandRunner {
        let mapped_inner = Arc::new(Map {
    ...
    pub fn output(
        &self,
        mut command: Command,
    ) -> Pin<Box<dyn Future<Output = io::Result<std::process::Output>> + Send>> {
...
pub trait InnerCommandRunner: Send + Sync {
...
impl InnerCommandRunner for RealCommandRunner {
    ...
    fn output(
        &self,
        command: Command,
    ) -> Pin<Box<dyn Future<Output = io::Result<async_process::Output>> + Send>> {
        let mut command: AsyncCommand = command.into();
...
impl NullCommandRunnerBuilder {
    ...
    pub fn cmd_full(
        &mut self,
        cmd: Command,
        out: impl Fn() -> Result<String, io::Error> + Send + Sync + 'static,
    ) -> &mut Self {
    ...
    pub fn build(&self) -> CommandRunner {
        let inner = Arc::new(NullCommandRunner {
...
impl InnerCommandRunner for NullCommandRunner {
    fn spawn(&self, command: Command) -> io::Result<Box<dyn Child + Send>> {
        ...
        let response = self
            ...
            .get(&key[..])
            .cloned()
            .unwrap_or(Arc::new(|| Ok(String::new())));
...
pub trait Child {
    ...
    fn wait(&mut self) -> Pin<Box<dyn Future<Output = Result<ExitStatus, io::Error>> + Send>>;
}
...
struct Map {
    inner: Arc<dyn InnerCommandRunner>,
    map_cmd: Arc<dyn Fn(Command) -> Command + Send + Sync>,
}
